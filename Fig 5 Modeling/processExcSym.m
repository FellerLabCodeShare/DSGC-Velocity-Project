function [vSort,vDSI] = processExcSym(dTable,indx,pIndx)
...
% Make exc symmetric by replacing pref dir exc w/ null dir exc, and adding
% back the proper spatial offset
symExcSort = [padExcSort(:,1:5) padExcSort(:,5:-1:1)];
newOffset = findOffset(symExcSort,padInhSort,respIndx);
...
for i = 1:10
    gExc = circshift(symExcSort(:,i),shiftBy(i));
    vTrace(:,i) = runModel(gExc,padInhSort(:,i));
end
...
if plotVoltage
    ...
    hF1 = plotCV(circshift(symExcSort(:,pIndx),shiftBy(pIndx)),padInhSort(:,pIndx),vTrace(:,pIndx));
    hF2 = plotCV(circshift(symExcSort(:,nIndx),shiftBy(nIndx)),padInhSort(:,nIndx),vTrace(:,nIndx));
