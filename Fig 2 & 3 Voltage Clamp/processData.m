function [pSort,pDSI,qSort,qDSI,pHold] = processData(dTable,indx,showPP,showSummary,offWindow)
...
if isExc %take min if exc, max if inh
    d = -d;
    pCur = max(d(respIndx,:));
    pHold = max(d(holdIndx,:)); % resp to no stim
%     pCur = min(d(respIndx,:));
%     pHold = min(d(holdIndx,:)); % resp to no stim
%     d = -d;
else
    pCur = max(d(respIndx,:));
    pHold = max(d(holdIndx,:)); % resp to no stim
